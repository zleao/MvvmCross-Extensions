    ...
    public class NotificationManager : INotificationService
    {
           ...
           where TMessage : INotificationMessage
        {
            ...
            lock (this)
            {
                ...
                {
                    MyTrace.Trace($"Adding oneway subscription (MessageType: '{messageType.Name}' Context: '{asyncSubscription.Context}' SubscriptionId: '{asyncSubscription.Id}')");
                    _oneWaySubscriptions[messageType].Add(asyncSubscription);
                }
                else
                {
                    MyTrace.Trace($"Adding new message type with oneway subscription (MessageType: {messageType.Name} Context: {asyncSubscription.Context} SubscriptionId: {asyncSubscription.Id})");
            ...
            where TResult : INotificationResult
        {
            ...
            lock (this)
            {
                if (_twoWaySubscriptions.ContainsKey(messageType))
                {
                    ...
                    {
                        MyTrace.Trace($"Replacing twoway subscription for specified context (MessageType: {messageType.Name} Context: {asyncSubscription.Context} SubscriptionId: {asyncSubscription.Id})");
                        _twoWaySubscriptions[messageType][context] = asyncSubscription;
                    }
                    else
                    {
                        MyTrace.Trace($"Adding twoway subscription for specified context (MessageType: {messageType.Name} Context: {asyncSubscription.Context} SubscriptionId: {asyncSubscription.Id})");
                        _twoWaySubscriptions[messageType].Add(context, asyncSubscription);
                    }
                }
                else
                {
                    MyTrace.Trace($"Adding new message type with twoway subscription for specified context (MessageType: {messageType.Name} Context: {asyncSubscription.Context} SubscriptionId: {asyncSubscription.Id})");
            ...
            if (subscriptionToken != null)
            {
                lock (this)
                {
                    ...
                    if (_oneWaySubscriptions.TryGetValue(subscriptionToken.MessageType, out oneWaySubscriptions))
                    {
                        ...
                        {
                            MyTrace.Trace($"Removing oneway subscription (MessageType: {subscriptionToken.MessageType} Context: {subscriptionToken.Context} SubscriptionId: {subscriptionToken.Id})");
                    ...
                    if (_twoWaySubscriptions.TryGetValue(subscriptionToken.MessageType, out twoWaySubscriptions))
                    {
                        ...
                        {
                            MyTrace.Trace($"Removing twoway subscription (MessageType: {subscriptionToken.MessageType} Context: {subscriptionToken.Context} SubscriptionId: {subscriptionToken.Id})");
        ...
        public void Unsubscribe(Type messageType)
        {
            lock (this)
            {
                ...
                if (_oneWaySubscriptions.TryGetValue(messageType, out oneWaySubscriptions))
                {
                    ...
                    {
                        MyTrace.Trace($"Removing oneway subscription (MessageType: {messageType} Context: {subscription.Context} SubscriptionId: {subscription.Id})");
                ...
                if (_twoWaySubscriptions.ContainsKey(messageType))
                {
                    MyTrace.Trace($"Removing all twoway subscriptions for type '{messageType}'");
        ...
        public async Task PublishAsync(INotificationMessage message, string context = AsyncSubscription.DefaultContext)
        {
            ...
            {
                MyTrace.Warning("NotificationMessage publishing not allowed - this normally suggests non-specific generic used in calling code");
                return;
            }
            ...
            {
                MyTrace.Trace($"Nothing registered for one-way messages of type '{messageType.Name}' with context '{context}'");
                return;
            }
            ...
            foreach (var subscription in oneWaySubscriptions)
            {
                ...
                {
                    MyTrace.Warning($"Found one-way subscription that is not a OneWayAsyncSubscription! ({messageType.Name})");
                }
                else
            ...
            if (!allSucceeded)
            {
                MyTrace.Trace("One or more oneway listeners failed - purge scheduled");
        ...
        public async Task DelayedPublishAsync(INotificationMessage message, string context = AsyncSubscription.DefaultContext, bool tryNormalPublish = false)
        {
            ...
            if (tryNormalPublish)
            {
                if (HasSubscriptionsForContext(messageType, context, SubscriptionDirectionEnum.OneWay))
                {
                    ...
                    MyTrace.Warning(string.Format("Failed to normal publish the one-way '{0}' message for context '{1}'. Adding it to delayed publish...", messageType.FullName, context));
            }
        ...
        public async Task PublishPendingNotificationsAsync(object currentPublisher, string context = AsyncSubscription.DefaultContext)
        {
            ...
            {
                MyTrace.Warning("PublishPendingNotificationsFor - Context is null or empty. Assuming default");
            ...
            if (_pendingNotifications.ContainsKey(context) && _pendingNotifications[context].Count() > 0)
            {
                ...
                if (msgsCount > 0)
                {
                    MyTrace.Trace($"Found {msgsCount} pending one-way message(s) with context '{context}' available for current publisher ({currentPublisher})");
                ...
                {
                    MyTrace.Trace($"Found {_pendingNotifications[context].Count()} one-way pending message(s) with context '{context}' but NONE available for current publisher ({currentPublisher})");
                }
            }
            else
            {
                MyTrace.Trace($"No pending one-way messages found with context {context}");
                return;
            }
        }
            ...
            where TResult : INotificationResult
        {
            ...
            if (twoWaySubscriptions == null)
            {
                MyTrace.Trace($"Nothing registered for two-way async messages of type '{messageType.Name}' with context '{context}'");
                return default(TResult);
            }

            if (twoWaySubscriptions.Count() > 1)
                MyTrace.Trace($"Found {twoWaySubscriptions.Count()} two-way messages for type '{messageType}' and context '{context}'. This should not happen. Going to use the first one...");

            var twoWaySubscription = twoWaySubscriptions.FirstOrDefault() as TwoWayAsyncSubscription<TResult>;
            if (twoWaySubscription == null)
            {
                MyTrace.Warning($"Found a two-way subscription that is not a TwoWayAsyncSubscription! (MessageType: {messageType.Name} Context: {context})");
                return default(TResult);
            }
            ...
            {
                MyTrace.Warning($"Two-way subscription is dead. Going to pruge it (MessageType: '{messageType.Name}' Context: '{context}' SubscriptionId: '{twoWaySubscription.Id}')");
        ...
        private void AddToPendingNotificationList(INotificationMessage message, string context)
        {
            ...
            if (string.IsNullOrEmpty(context))
            {
                MyTrace.Warning($"AddToPendingNotificationList - Context of message type '{messageType.Name}' is null or empty. Assuming default");
                context = AsyncSubscription.DefaultContext;
            }
            ...
            lock (this)
            {
                ...
                MyTrace.Trace($"AddToPendingAsyncNotificationList - Adding one-way message of type '{messageType.Name}' with context '{context}' to pending async notification list");
                            ...
                            foreach (var type in messageTypes)
                            {
                                ...
                                if (_oneWaySubscriptions.TryGetValue(type, out oneWaySubscriptionsForType) && oneWaySubscriptionsForType != null)
                                {
                                    foreach (var subscription in oneWaySubscriptionsForType.ToArray())
                                    {
                                        ...
                                        {
                                            MyTrace.Trace($"Purging one-way subscription (MessageType: {type} Context: { subscription.Context} SubscriptionId: {subscription.Id})");
                            ...
                            foreach (var type in messageTypes)
                            {
                                ...
                                if (_twoWaySubscriptions.TryGetValue(type, out twoWaySubscriptionsForType) && twoWaySubscriptionsForType != null)
                                {
                                    foreach (var context in twoWaySubscriptionsForType.Keys.ToArray())
                                    {
                                        ...
                                        {
                                            MyTrace.Trace($"Purging two-way subscription (MessageType: {type} Context: {context} SubscriptionId: {twoWaySubscriptionsForType[context].Id})");
