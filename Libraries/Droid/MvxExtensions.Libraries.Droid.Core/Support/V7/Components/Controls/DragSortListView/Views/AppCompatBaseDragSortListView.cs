...
using MvxExtensions.Libraries.Droid.Core.Support.V7.Components.Controls.DragSortListView.Runnables;
...
{
    public abstract class AppCompatBaseDragSortListView : ListViewCompat
    {
        #region Constants

        // Drag flag bit. 
        // Floating View can move in the positive x direction.
        public readonly int DRAG_POS_X = 0x1;

        // Drag flag bit. Floating View can move in the negative
        // x direction.
        public readonly int DRAG_NEG_X = 0x2;
        ...
        /// </summary>
        private AppCompatDragScroller _dragScroller;

        private AppCompatRemoveAnimator _removeAnimator;

        private AppCompatLiftAnimator _liftAnimator = null;

        private AppCompatDropAnimator _dropAnimator;

        #endregion

        #region Constructor

        public AppCompatBaseDragSortListView(Context context, IAttributeSet attrs)
            : base(context, attrs)
        {
            ...
            _scrollProfile = new AppCompatLinearDragScrollProfile(this);

            ParseAttributes(context, attrs);

            _dragScroller = new AppCompatDragScroller(this);

            float smoothness = 0.5f;
            if (_removeAnimDuration > 0)
            {
                _removeAnimator = new AppCompatRemoveAnimator(this, smoothness, _removeAnimDuration);
            }
            if (_dropAnimDuration > 0)
            {
                _dropAnimator = new AppCompatDropAnimator(this, smoothness, _dropAnimDuration);
            }
        ...
        private void ParseAttributes(Context context, IAttributeSet attrs)
        {
            ...
            var dragSortListViewAttrsId = (int[])SafeGetFieldValue(styleable, "AppCompatDragSortListView", new int[0]);
            TypedArray a = context.ObtainStyledAttributes(attrs, dragSortListViewAttrsId, 0, 0);

            _trackDragSort = a.GetBoolean((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_track_drag_sort", 0), DEFAULT_TRACK_DRAG_SORT);
            if (_trackDragSort)
                _dragSortTracker = new DragSortTracker();

            _itemHeightCollapsed = Math.Max(1, a.GetDimensionPixelSize((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_collapsed_height", 0), DEFAULT_ITEM_HEIGHT_COLLAPSED));

            // alpha between 0 and 255, 0=transparent, 255=opaque
            _floatAlpha = a.GetFloat((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_float_alpha", 0), DEFAULT_FLOAT_ALPHA);
            _currFloatAlpha = _floatAlpha;

            _dragEnabled = a.GetBoolean((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_drag_enabled", 0), DEFAULT_DRAG_ENABLED);

            _slideRegionFrac = Math.Max(0.0f, Math.Min(1.0f, 1.0f - a.GetFloat((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_slide_shuffle_speed", 0), DEFAULT_SLIDE_SHUFFLE_SPEED)));
            _animate = _slideRegionFrac > 0.0f;

            SetDragScrollStart(a.GetFloat((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_drag_scroll_start", 0), DEFAULT_DRAG_UP_SCROLL_START_FRAC));

            _maxScrollSpeed = a.GetFloat((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_max_drag_scroll_speed", 0), DEFAULT_MAX_SCROLL_SPEED);

            _removeAnimDuration = a.GetInt((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_remove_animation_duration", 0), DEFAULT_ANIMATION_DURATION_MILISECONDS);

            _dropAnimDuration = a.GetInt((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_drop_animation_duration", 0), DEFAULT_ANIMATION_DURATION_MILISECONDS);

            bool useDefault = a.GetBoolean((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_use_default_controller", 0), DEFAULT_USE_DEFAULT_CONTROLLER);
            if (useDefault)
            {
                bool removeEnabled = a.GetBoolean((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_remove_enabled", 0), DEFAULT_CONTROLLER_REMOVE_ENABLED);
                RemoveModeEnum removeMode = (RemoveModeEnum)a.GetInt((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_remove_mode", 0), (int)DEFAULT_CONTROLLER_REMOVE_MODE);
                bool sortEnabled = a.GetBoolean((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_sort_enabled", 0), DEFAULT_CONTROLLER_SORT_ENABLED);
                DragInitModeEnum dragInitMode = (DragInitModeEnum)a.GetInt((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_drag_start_mode", 0), (int)DEFAULT_CONTROLLER_DRAG_INIT_MODE);
                int dragHandleId = a.GetResourceId((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_drag_handle_id", 0), DEFAULT_CONTROLLER_DRAG_HANDLE_ID);
                int flingHandleId = a.GetResourceId((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_fling_handle_id", 0), DEFAULT_CONTROLLER_FLING_HANDLE_ID);
                int clickRemoveId = a.GetResourceId((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_click_remove_id", 0), DEFAULT_CONTROLLER_CLICK_REMOVE_ID);
                var bgColor = a.GetColor((int)SafeGetFieldValue(styleable, "AppCompatDragSortListView_float_background_color", 0), DEFAULT_CONTROLLER_BACKGROUND_COLOR);

                var controller = new AppCompatDragSortController(this, dragHandleId, dragInitMode, removeMode, clickRemoveId, flingHandleId);
        ...
        private void ContinueDrag(int x, int y)
        {
            ...
            if (minY > _lastY && minY > _downScrollStartY && currentScrollDir != AppCompatDragScroller.DOWN)
            {
                // dragged down, it is below the down scroll start and it is not
                // scrolling up

                if (currentScrollDir != AppCompatDragScroller.STOP)
                {
                ...
                _dragScroller.StartScrolling(AppCompatDragScroller.DOWN);
            }
            else if (maxY < _lastY && maxY < _upScrollStartY && currentScrollDir != AppCompatDragScroller.UP)
            {
                ...
                // start scrolling up
                _dragScroller.StartScrolling(AppCompatDragScroller.UP);
            }
